...
The `rest-dynamic-controller` is an operator instantiated by the [oasgen-provider](../oasgen-provider/) to manage Custom Resources whose Custom Resource Definitions (CRDs) are generated by the `oasgen-provider`.

Extended and detailed information about the overall setup and usage of the `oasgen-provider` and `rest-dynamic-controller` can be found in the repo's [doc bundle](../../docs/index.md). It is recommended to refer to that document as this README is just a concise summary focused on the `rest-dynamic-controller`.
...
1. The `rest-dynamic-controller` instance checks if the remote resource exists by following the instructions defined in the [`RestDefinition`](../../docs/restdefinition-crd-reference.md).
...
| REST_CONTROLLER_WORKERS | Number of worker threads | `5` |
...
| REST_CONTROLLER_MAX_ERROR_RETRY_INTERVAL | The maximum interval between retries when an error occurs. This should be less than the half of the resync interval. | `90s` |
...
| REST_CONTROLLER_SERVICEACCOUNT_NAME / _NAMESPACE | The controller's own ServiceAccount identity (injected by the rendered ConfigMap; used to authenticate delegated `*ApiRef` verbs) | - |
| REST_CONTROLLER_SERVICEACCOUNT_TOKEN_PATH | Where the ServiceAccount token is read from | in-pod default |
| URL_SNOWPLOW / URL_AUTHN | Snowplow `/call` and authn endpoints — required only when a RestDefinition uses `observeApiRef`/`createApiRef`/`updateApiRef`/`deleteApiRef` | `""` |
| OTEL_ENABLED / OTEL_TRACING_ENABLED | OTLP metrics / trace export | `false` |
| KUBECONFIG | Out-of-cluster development only | `""` (in-cluster) |

Each variable has a CLI-flag twin; defaults verified against `main.go`. The canonical
configuration reference (including the chart values that parameterize every generated
controller) is [docs/configuration.md](../../docs/configuration.md).
